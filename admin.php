...
<body>
    ...
    <!-- Add User Modal -->
<!-- Add User Modal - Include this in your admin.php page -->
<div class="modal fade" id="addUserModal" tabindex="-1" aria-labelledby="addUserModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div id="addUserAlert" class="alert" role="alert" style="display: none;"></div>
                <form id="addUserForm">
                    <div class="mb-3">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="standard_balance" class="form-label">Standard Balance</label>
                            <input type="number" class="form-control" id="standard_balance" name="standard_balance" value="0" step="0.01" min="0">
                            <div class="invalid-feedback">Please enter a valid amount</div>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="premium_balance" class="form-label">Premium Balance</label>
                            <input type="number" class="form-control" id="premium_balance" name="premium_balance" value="0" step="0.01" min="0">
                            <div class="invalid-feedback">Please enter a valid amount</div>
                        </div>
                    </div>
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="login_alerts" name="login_alerts">
                        <label class="form-check-label" for="login_alerts">Enable Login Alerts</label>
                    </div>
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="password_changes" name="password_changes">
                        <label class="form-check-label" for="password_changes">Allow Password Changes</label>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add a variable to track form submission status
    ...
    function validateUsername(input) {
        ...
        if (!isValid && input.value.trim() !== '') {
            ...
            const feedbackElement = input.nextElementSibling;
            if (feedbackElement && feedbackElement.classList.contains('invalid-feedback')) {
                feedbackElement.textContent = 'Username must be 3-20 characters and contain only letters, numbers, and underscores';
            }
            return false;
        } else {
            input.classList.remove('is-invalid');
    ...
    addUserForm.addEventListener('submit', function(e) {
        ...
        if (!emailValid || !usernameValid) {
            showNotification('Validation Error', 'Please correct the form errors before submitting', 'warning');
            return;
        }
        ...
        // Add the action parameter to identify this as an add user request
        ...
        .then(data => {
            ...
            if (data.success) {
                // Show the success alert inside the modal
                alertDiv.className = 'alert alert-success';
                alertDiv.textContent = data.message || 'User added successfully!';
                alertDiv.style.display = 'block';
                
                // Reset the form after successful submission
                addUserForm.reset();
                
                // First show the notification toast
                showNotification('Success', data.message || 'User added successfully!', 'success');
                
                // IMPORTANT: Wait before closing modal to ensure notification is visible
                setTimeout(() => {
                    ...
                    // Reload the page to refresh the user list after a short delay
                    setTimeout(() => {
                        window.location.reload();
                    }, 1000);
                    
                    // Hide the alert after the modal is closed
                    setTimeout(() => {
                ...
                alertDiv.textContent = data.message || 'Failed to add user. Please try again.';
                alertDiv.style.display = 'block';
                showNotification('Error', data.message || 'Failed to add user', 'danger');
            }
        })
        .catch(error => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Check username in real-time when user stops typing
    if (usernameInput) {
        let usernameTimer;
    ...
    // Check email in real-time when user stops typing
    if (emailInput) {
        let emailTimer;
...
// Function to show toast notifications with improved visibility
function showNotification(title, message, type) {
    // Create toast container if it doesn't exist
    let toastContainer = document.getElementById('toast-container');
    if (!toastContainer) {
        ...
        toastContainer.id = 'toast-container';
        toastContainer.className = 'position-fixed top-0 end-0 p-3';
        toastContainer.style.zIndex = '1080'; // Increased z-index to ensure it appears above modal
        document.body.appendChild(toastContainer);
    }
    ...
    // Create toast element with improved visibility
    const toastEl = document.createElement('div');
    toastEl.id = toastId;
    toastEl.className = `toast align-items-center text-white bg-${type} border-0`;
    toastEl.setAttribute('role', 'alert');
    ...
    // Ensure the toast is more visible with styling
    ...
    // Initialize and show toast with longer delay
    const toast = new bootstrap.Toast(toastEl, {
        autohide: true,
        delay: 7000 // Increased from 5000 to 7000 ms for better visibility
    });
    toast.show();
    ...
    toastEl.addEventListener('hidden.bs.toast', function() {
        ...
        // Add this JavaScript to your admin.php page
        document.addEventListener('DOMContentLoaded', function() {
            // Only initialize if the token display element exists
            if (document.getElementById('dynamic-token-display')) {
                initTokenRefresh();
            }
        });
        ...
        function initTokenRefresh() {
            ...
            let countdown = 60; // Default 60 second tokens
            ...
            const countdownInterval = setInterval(() => {
                ...
                if (timerDisplay) {
                    ...
                    // Visual indication that token is about to expire
                    if (countdown <= 10) {
                ...
                // When countdown reaches 0, fetch new token
                if (countdown <= 0) {
                    fetchCurrentToken();
                    countdown = 60;
                }
        ...
        function fetchCurrentToken() {
            const formData = new FormData();
            formData.append('action', 'get-current-token');
            
            fetch('admin_login.php', {
                method: 'POST',
                body: formData,
                credentials: 'same-origin' // Important for session data
            })
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('dynamic-token-display').textContent = data.token;
                    
                    // Update countdown timer if time remaining is not 60 seconds
                    // (in case we're fetching mid-cycle)
